...
function uploadGalleryFile($file, $type, $conn, $caption = '', $thumbnail = null) {
    ...
    // Validate file type
    if ($type == 'image' && !in_array($file_ext, $allowed_image_types, true)) {
        return ['status' => 'error', 'message' => 'Invalid image file type (allowed: ' . implode(', ', $allowed_image_types) . ')'];
    }
    
    if ($type == 'video' && !in_array($file_ext, $allowed_video_types, true)) {
    ...
    if ($mime === '' || !in_array($mime, $allowed_mime, true)) {
        return ['status' => 'error', 'message' => 'Invalid file content type'];
    }
    if ($type == 'image' && @getimagesize($file_tmp) === false) {
